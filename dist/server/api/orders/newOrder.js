...
const createOrder = async (order) => {
    ...
    try {
        ...
        const serverOrder = Object.assign(Object.assign({}, order), { id: stringId });
...
const getOrderErrorsObject = async ({ zone, cart, phone, name, deliveryType, locality, street, house, deliveryTimeType, deliveryTime, }) => {
    const result = {};
    // Sum validation
    ...
    if (deliveryType === 'На указанный адрес') {
        if (zone !== 'Талдом' && !(locality === null || locality === void 0 ? void 0 : locality.trim())) {
            isValid = false;
